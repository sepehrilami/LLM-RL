...
a2c_manager.load_model('save_model/manager_networks_20nodes_39')
...
for episode in range(training_episode):
    ...
    g = nx.erdos_renyi_graph(num_agent, p=0.2)
    # g = nx.complete_graph(num_agent)
    ...
    for step in range(game_length):
        for (index1, index2) in link_list:
            if step < 2:
                ...
                prompt_1 = build_prompt(intervention=intervention1)
                prompt_2 = build_prompt(intervention=intervention2)
                ...
                # change this part for different baseline
                ...
                # # agent ratio baseline
                # intervention1 = 1
                # intervention2 = 1

                # # network ratio baseline
                # intervention1 = 2
                # intervention2 = 2
    ...
    print(f'Duration: {round(time.time() - initial_time, 2)}, Round:{episode}, '
          f'C ratio: {number_C/total_number}')

np.save('outputs/whole_agent_ratio_list_record', np.array(whole_agent_ratio_list_record))
np.save('outputs/whole_observation_list_record', np.array(whole_observation_list_record))
np.save('outputs/whole_intervention_list_record', np.array(whole_intervention_list_record))
np.save('outputs/whole_adjacency_matrix_record', np.array(whole_adjacency_matrix_record))
